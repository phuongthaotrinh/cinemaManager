...
import {formatDate2, formatRunTimeToDate } from '../../ultils';
type Props = {
    item: any,
}
    ...
    return (
        <div className="w-[250px] h-[350px] pl-2  bg-black inline-block cursor-pointer relative p-2 ">
            ...
            <div className="absolute w-full top-0 left-0 h-full hover:bg-black/80  opacity-0 hover:opacity-100 text-white">
                <p onClick={() => showModal(item?._id)} className='white-space-normal text-xs md:text-sm font-bold flex justify-center items-center h-full text-center'>{item?.name}</p>
                <Modal title={
                    ...
                    <i> <b>Thể loại: </b>{item?.movieTypeId && item?.movieTypeId?.map((item: any) => <span key={item.id}>{item?.movieName.concat(" ")}</span>)}</i> <br />
