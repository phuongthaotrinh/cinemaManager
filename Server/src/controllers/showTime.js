...
export const create = async (req, res) => {
    try {
        ...
        if (existShowTime.length) return res.status(400).json({
...
export const getShowTimeByDate = async (req, res) => {
   try {
    ...
    const showTimesByMoiveId = await ShowTime.find({movieId: req.params.id},'-movieTypeId').populate({
        path: 'movieId',
        select: 'name _id'
    ...
    return res.json(showTimesByMoiveId);
